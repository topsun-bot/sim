...
| 维度 | MuJoCo（默认） | Isaac Sim 5.1（profile: isaac） |
|------|----------------|----------------------------------|
| 镜像 | `sim-replay:mujoco` (~1 GB) | `sim-replay:isaac`（基于 NGC 5.1.0） |
| GPU | EGL 无头渲染 | 必须 NVIDIA GPU |
| Python | 3.11 | 3.11（kit `python.sh`） |
| 物理 | MuJoCo 原生 | 当前 MuJoCo 委托，原生 Isaac 待 Phase 3 |
| 可视化 | `-v` 3D + 相机 | 容器内受限，建议 MuJoCo profile |
...
```bash
replay run -v              # 真实时间
replay run -v --fast       # 全速 + 可视化
```

回放时同步显示 MuJoCo 3D 视窗与 scene/wrist 相机画面；按 **Q** / **Esc** 可提前结束。无显示器时自动降级，不中断回放。
...
- **Phase 1**：MuJoCo 物理回放 MVP、Docker 双后端、可视化（已完成）
- **Phase 2**：批处理、域随机化、提升 sim_success
- **Phase 3**：Isaac Sim 原生后端、L2 回归门禁
